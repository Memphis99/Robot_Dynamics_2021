...
for k = 1:length(q)
    M = M + I_Jp{k}'*m{k}*I_Jp{k} + I_Jr{k}'*R_Ik{k}*k_I_s{k}*R_Ik{k}'*I_Jr{k};
end
...
for k=1:size(q)
    b=b + I_Jp{k}.'*m{k}*dAdt(I_Jp{k}, q, dq)*dq + ...
      (I_Jr{k}).'*R_Ik{k}*k_I_s{k}*(R_Ik{k}).'*dAdt(I_Jr{k}, q, dq)*dq + ...
      (I_Jr{k}).'*cross(I_Jr{k}*dq, R_Ik{k}*k_I_s{k}*(R_Ik{k}).'*I_Jr{k}*dq);
end
