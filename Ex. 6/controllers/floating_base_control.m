...
dw_star_c=0;
A_c = [J_c; zeros(4, 10); zeros(7, 10)];
b_c = [dw_star_c-dJ_c*dq];

% Body motion
kp = 1;
kd = 2*sqrt(kp);
dw_star_B=kp*(p_star_B - p_B) + kd*(w_star_B - w_B);
A_b = [J_B; zeros(4, 10); zeros(7, 10)];
b_b = [dw_star_B-dJ_B*dq];

% No end-effector motion
dw_star_EE=0;
A_ee = [J_EE; zeros(4, 10); zeros(7, 10)];
b_ee = [dw_star_EE-dJ_EE*dq];
